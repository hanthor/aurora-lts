...
Aurora LTS is a container-based operating system image built on CentOS Stream 10 using bootc technology. It creates bootable container images that can be converted to disk images, ISOs, and VM images. It features the KDE Plasma desktop environment.
...
- **Build validation workflow**:
  ...
  3. Test build with `just build aurora lts` (120+ minute timeout)
...
4. Test with full build: `just build aurora lts` (120+ minute timeout)
...
2. **Build test**: `just build aurora lts` (full 120+ minute build)
...
just build aurora lts        # Standard build
just build aurora lts 1 0 0  # With DX (developer tools)
just build aurora lts 0 1 0  # With GDX (GPU/AI tools)
...
4. Retry with full timeout: `just build aurora lts` (120+ minutes)
